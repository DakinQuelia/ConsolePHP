...
/*======================================================================*\
||  Corona CMS						                                    ||
||  Fichier     : Console\Commands\MakeControllerCommand.php     		||
||  Version     : 1.0.0.                                	            ||
...
class MakeControllerCommand extends Command
{
	...
	**/
	protected function configure(): void
	{
		...
		$this->setDescription("Création d'un nouveau contrôleur");
		$this->setHelp("Cette commande permet de créer un contrôleur.");
		...
		$this->addArgument('template_view', InputArgument::REQUIRED, "Nom du fichier modèle");
	}
	...
	**/
	protected function initialize(InputInterface $input, OutputInterface $output): void
    {
		...
		// Fichiers modèles
		$this->controller_file = $this->skeleton_dir . DIRECTORY_SEPARATOR . 'Controller' . DIRECTORY_SEPARATOR . 'controller.tpl.php';
		$this->controller_view_file = $this->skeleton_dir . DIRECTORY_SEPARATOR . 'Controller' . DIRECTORY_SEPARATOR . 'controller.html';
	...
	**/
    ...
    {
		// Informations
		$this->io->title("Création d'un contrôleur");
        $this->io->text([
    	  "Si vous préférez utiliser la ligne de commande plutôt que l'installateur",
          "Voici la commande :",
          '',
          ' $ php bin/console make:controller nom_module auteur_controleur nom_controleur namespace ',
          ''
        ]);
		...
		$template_view = $this->io->ask(sprintf("Nommez le fichier template (sans l'extension) [Exemple :: %s] ", "<comment>index</comment>"), 'index');
		$input->setArgument('template_view', $template_view);
	}
	...
	**/
	protected function execute(InputInterface $input, OutputInterface $output): int
	{
		...
		$data = [
			...
			'template_view'		=> $input->getArgument('template_view')
		];
		...
		if ($confirm)
		{
			...
			$controller_view_file = $new_controller_view_dir . $data['template_view'] . ".html";
			...
			$output->writeln(sprintf('  <fg=green>Créé :</> %s', $this->relativizePath($controller_view_file )));
		}
	...
	**/
	public function generateController(array $data): void
	{
		...
		$controller_tpl = str_replace('{{templatename}}', $template, $controller_tpl);

		$controller_view = str_replace('{{layout}}', $filelayout, $controller_view);

		$new_controller_view = file_put_contents($this->view_dir . DIRECTORY_SEPARATOR . $data['module_name'] . DIRECTORY_SEPARATOR . $template . ".html", $controller_view);
